...
- Mermaid の GUI デザインモードは React Flow ベースのカスタムノードを採用し、処理/入出力/サブルーチンは角丸のない矩形、分岐はハンドルが角に位置する菱形として描画します。エッジ接続ハンドルは上下方向（上が target、下が source）を維持しつつ、自動整列ボタンから縦／横いずれのレイアウトにも再配置できます。
...
- マップタブでは Deck.gl と OpenStreetMap タイル（`https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png`）を利用し、緯度経度・GeoJSON・WKT 列から点／ライン／ポリゴン／3D カラムを描画します。
...
- マップタブは Deck.gl を用いて統合データ／クエリ結果／個別ファイルを切り替えながら OpenStreetMap タイル上に可視化し、カテゴリ別の色分けや 3D カラム高さの集計設定を保持します。
