...
- **SQLクエリ実行** - AlasQLによる強力なデータ操作（結果は即座に表・チャートへ切替可能）
- **統計情報表示** - pandas.describe()相当の統計サマリー
- **クエリ結果からのチャート作成** - 棒／折れ線／円／散布図／ヒストグラム／積み上げ棒／線形回帰（グループ別回帰線対応）／バブル／サンバースト／ガント
...
9. **データ分析**：CSV/TSV/JSON/YAML/Parquetファイルを開いた場合、分析イメージから「クエリ」タブでSQLを実行し、結果テーブルの「チャート」ビューに切り替えるだけで可視化できます（ガントチャートはタスク名／開始日／終了日フィールドが必要です）。
